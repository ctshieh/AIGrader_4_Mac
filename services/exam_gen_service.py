...
# -*- coding: utf-8 -*-
# Module-Version: v2026.01.13-AlignFix
#
# Changes:
# 1. [Layout] Answer Sheet Alignment: Standard questions (no sub-qs) now force the Question Number (Item Label) 
#    to appear ABOVE the answer box using '\item \mbox{} \par'. This matches the user's requirement.
# 2. [Layout] Maintained Answer Sheet 'Suffix' in \large and Title in \Large.
# 3. [Fix] Retained [Q1] tag at (0,0) inside the box.
...
class ExamBuilder:
    ...
    def generate_tex_source(self, header_info, questions, exam_uuid="UNK", has_logo=False, has_marketing_qr=False):
        ...
        font_base = "/app/fonts/"
        if not os.path.exists(font_base): font_base = "/usr/share/fonts/truetype/custom/"
        tex.append(rf"\setCJKmainfont[Path={font_base}, AutoFakeBold=3, AutoFakeSlant=.2]{{cwTeXQMing-Medium.ttf}}")
        tex.append(rf"\setCJKsansfont[Path={font_base}, AutoFakeBold=3, AutoFakeSlant=.2]{{cwTeXQHei-Bold.ttf}}")
        tex.append(rf"\setCJKmonofont[Path={font_base}, AutoFakeBold=3, AutoFakeSlant=.2]{{cwTeXQYuan-Medium.ttf}}")
        ...
        # 1. Full Header (For Answer Sheet)
        def render_full_header(title_suffix=""):
            ...
            if is_compact:
                ...
                if full_subtitle: tex.append(rf"{{\small {full_subtitle}}} & \\")
                tex.append(r"\end{tabularx}")
                tex.append(r"\vspace{0.2cm}")
                tex.append(r"\noindent \begin{tcolorbox}[colframe=black, colback=white, boxrule=0.8pt, arc=2pt, left=4pt, right=4pt, top=2pt, bottom=2pt]")
                tex.append(r"\small")
                tex.append(rf"\textbf{{{lbl_dept}：}}\underline{{\hspace{{1.5cm}}}} \quad \textbf{{{lbl_name}：}}\underline{{\hspace{{1.5cm}}}} \quad")
                tex.append(rf"\textbf{{{lbl_id}：}} \underline{{\hspace{{2.5cm}}}} \quad \textbf{{{lbl_seat}：}} \underline{{\hspace{{1.0cm}}}} \quad")
                tex.append(rf"\textbf{{{lbl_score_box}：}} \underline{{\hspace{{1.0cm}}}}")
        ...
        # 2. Branding Header (For Question Paper)
        ...
        def render_questions(mode="full"): 
            ...
            for q_idx, q in enumerate(questions):
                ...
                # [PART A] Render Main Question Text (Only in full or text_only)
                ...
                # [PART B] Item Start for Box Only
                if mode == "box_only":
                    tex.append(r"\item ") 

                # [PART C] Render Sub-questions / Options / Boxes
                ...
                if should_render_structure:
                    if sub_qs:
                        ...
                        # Standard Question (No Options, No Sub-Qs)
                        if mode == "full":
                            ...
                            # [FIX 2] Use \mbox{} \par to FORCE the Item Label (Number) to be on the line ABOVE the box
                            label_str = f"[Q{q_idx+1}]"
                            tex.append(r"\mbox{} \par\vspace{2pt}") # This moves the box to the next line after the number
                            tex.append(r"\noindent\begin{tikzpicture}")
                            tex.append(r"\draw[line width=0.8pt, color=black] (0,0) rectangle (\linewidth, -" + box_h + r");")
                            tex.append(r"\node[anchor=north west, inner sep=3pt] at (0, 0) {\small \textbf{" + label_str + r"}};")
                            tex.append(r"\end{tikzpicture}\par\vspace{0.3cm}")
    ...
    def generate_pdf(self, header_info, questions, user=None):
        ...
        marketing_url = None
        if user and user.plan in ['enterprise', 'premium']:
        ...
        default_logo = "/app/assets/default_logo.png"
        has_logo = (user_logo and os.path.exists(user_logo)) or os.path.exists(default_logo)
        
        source = self.generate_tex_source(header_info, questions, exam_uuid=exam_id, has_logo=has_logo, has_marketing_qr=bool(marketing_url))
        pdf_bytes = self.compile_tex_to_pdf(source, exam_id, system_qr_content, marketing_url, user)
    ...
    def compile_tex_to_pdf(self, tex_source, exam_id, system_qr_content, marketing_url, user):
        with tempfile.TemporaryDirectory() as temp_dir:
            # P-Series: Answer Sheet
            for i in range(1, 15): 
                self._generate_qr_file(f"{system_qr_content}-P{i}", os.path.join(temp_dir, f"qrcode_p{i}.png"))
                # Q-Series: Question Paper
            ...
            if user_logo and os.path.exists(user_logo): shutil.copy(user_logo, target_logo)
            elif os.path.exists(default_logo): shutil.copy(default_logo, target_logo)
